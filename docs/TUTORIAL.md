...
- **InvokeAI と画面を分割して使うことを想定しています。** 片側に InvokeAI のビューア、もう片側に PromptMosaic を置くと、生成結果を見ながらプロンプト編集・履歴管理・再生成ができます。
...
![InvokeAI データ取得ウィザード](images/invoke_setup_starting.png)
...
テンプレートを取得していないベースモデルで生成しようとすると、「テンプレートが有りません」と表示されます。その場合は、InvokeAI 側でそのベースモデルの画像を生成してから、もう一度ウィザードで **取得** してください。
