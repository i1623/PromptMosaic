...
1. **Invoke 側で、使いたいベースモデルの標準 txt2img 画像を 1 枚生成します。**
   - **対応 LoRA を必ず 1 つ以上使ってください。** PromptMosaic はテンプレート内の LoRA 経路を「見本」として再利用します。
   - 通常 CFG とネガティブプロンプトに対応するモデルでは、**CFG を 1 より大きくし、空でないネガティブプロンプトを入力**してください。
   - ControlNet・IP-Adapter・参照画像・img2img・inpaint・Refinerなどは無効にしてください。
...
取得条件を満たすテンプレートは同じベースモデルへ複数登録でき、**名前で区別**します（不要なものは 🗑 で削除）。取得に失敗した場合は、取得画面のプロンプト欄に表示される操作案内に従って Invoke で再生成してください。「詳細」には不足している経路などの技術情報も表示されます。
