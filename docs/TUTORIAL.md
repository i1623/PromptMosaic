...
PromptMosaic は、**プロンプトを「タイル」として組み立て、InvokeAI へ生成を依頼する**ローカル GUI ツールです。
...
PromptMosaic フォルダ内で、次の 2 つのバッチファイルを使います。
...
成功すると `.venv` フォルダが作られ、`Install complete.` と表示されます。
...
- **左ペイン（タグブラウザ）** — `◀ タグ` ボタンで開閉。タブで **タグ / モデル / LoRA / 文章** を切り替え。ここから中央へドラッグ＆ドロップして素材を追加します。
...
1. 左ペインの **🏷 タグ** タブを開きます。
2. 使いたいタグを中央ペインの **Positive ブロック** へドラッグ＆ドロップします。タグは「タイル」として並びます。
3. 直接入力したいときは、ブロック下部の入力欄にタグを打って **追加** を押します。
4. タイルは次のように操作できます。
   - **ドラッグで並べ替え**
   - **クリックで ON / OFF**（一時的に外す）
   - **強調（emphasis）の増減**でそのタグの効きを強める／弱める
5. ネガティブを入れたい場合は **Negative ブロック** に同じ要領でタイルを置きます。

> 💡 日本語のタグでも、翻訳 LLM（LM Studio）を設定しておけば「単語翻訳追加 / 文章翻訳追加」で英語タイルに変換して追加できます（任意機能）。
...
- **暗号化バックアップ**で大切なプロンプトと履歴を守る → [MANUAL: バックアップ](MANUAL.md#バックアップ--リストア)
