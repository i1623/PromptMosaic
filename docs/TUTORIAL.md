...
GitHub から **Code → Download ZIP** でリポジトリ全体をダウンロードして展開するか、`git clone` で取得してください。`install_windows.bat` だけを単体で保存しても、必要なファイルが足りないためインストールできません。

展開した PromptMosaic フォルダ内で、次の 2 つのバッチファイルを使います。
...
`install_windows.bat` はダブルクリックで実行できます。成功すると `.venv` フォルダが作られ、`Install complete.` と表示されます。失敗した場合も画面は閉じず、理由が表示されます。
