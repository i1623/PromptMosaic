...
- 🖥️ **InvokeAI との分割画面運用** — InvokeAI のビューアを横で開き、PromptMosaic でプロンプト編集・履歴管理・再生成
- 🧩 **二言語表示のタイル編集** — 英語プロンプトと日本語などの表示名を並べて見ながら、単語・文章をタイルとして D&D で並べ替え・強調・ON/OFF。よく使う組み合わせはタイルグループ化し、シャッフルやランダム選択で差分生成に利用できます。
