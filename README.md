...
InvokeAI と画面を分割して並べ、InvokeAI 側で生成結果を見ながら PromptMosaic 側でプロンプトを編集・再生成する使い方を想定しています。英語プロンプトと日本語などの表示名を同時に扱いながら、プロンプトを「タイル」として組み立てられることを重視しています。LM Studio などのローカル LLM を翻訳用に設定すると、日本語の単語や文章を英語プロンプトへ変換してタイル化できます。画像生成そのものは InvokeAI が行います。

- 🖥️ **InvokeAI との分割画面運用** — InvokeAI のキャンバス / ビューアを横に置き、PromptMosaic でプロンプト編集・履歴管理・再生成
- 🧩 **二言語表示のタイル編集** — 英語プロンプトと日本語などの表示名を並べて見ながら、単語・文章をタイルとして並べ替え・強調・ON/OFF
...
まず **GitHub の Code → Download ZIP** でリポジトリ全体をダウンロードして展開するか、`git clone` してください。`install_windows.bat` だけを単体でダウンロードしてもインストールできません。
...
`install_windows.bat` はダブルクリックでも実行できます。失敗した場合は黒い画面に理由が表示されるので、内容を確認してからキーを押して閉じてください。
